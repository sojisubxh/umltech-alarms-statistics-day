 ...
 * @since JDK1.8
 */
public class DayStatusApp implements Serializable {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job hbaseJob = Job.getInstance(conf);
        hbaseJob.setInputFormatClass(TableInputFormat.class);
        hbaseJob.setOutputFormatClass(TableOutputFormat.class);
        hbaseJob.getConfiguration().set(TableOutputFormat.OUTPUT_TABLE, TABLE_NAME_ALARM);
        hbaseJob.getConfiguration().set(TableInputFormat.INPUT_TABLE, TABLE_NAME_ALARM);
        ...
        JavaPairRDD<ImmutableBytesWritable, Put> hbaseRdd = resultRdd.mapPartitionsToPair((PairFlatMapFunction<Iterator<Tuple2<String, String>>, ImmutableBytesWritable, Put>) result -> {
            List<Tuple2<ImmutableBytesWritable, Put>> resultlist = new ArrayList<>();
            while (result.hasNext()) {
                String value = result.next()._2();
                ...
                list.forEach(alarmDetail -> {
                    if (alarmDetail != null) {
                        ...
                        resultlist.add(new Tuple2<>(new ImmutableBytesWritable(), put));
                    }
                });
            }
            return resultlist;
        ...
        resultRdd.foreachPartition((VoidFunction<Iterator<Tuple2<String, String>>>) result -> {
    ...
    private static String[] argsForTest() {
        String inputDir = "hdfs://cdh2:8020/spark/can/2019/03/06/part-000151552879020000";
        String outputDir = "hdfs://cdh-master2:8020/data/spark/alarmStatus";
